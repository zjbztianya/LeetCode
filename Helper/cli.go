...
func (c *CLI) Run() {
	...
	bathGenCmd := flag.NewFlagSet("batch", flag.ExitOnError)
	minNum := bathGenCmd.Int("l", 0, "请输入你想要准备的题目的最小题号")
	maxNum := bathGenCmd.Int("r", 0, "请输入你想要准备的题目的最大题号")
	...
	case "batch":
		err := bathGenCmd.Parse(os.Args[2:])
		check(err)
		buildMultiProblemDir(*minNum, *maxNum)
	case "task":
